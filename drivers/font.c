/* =============================================================================
 * GravityOS — Font Driver (8x8)
 * ============================================================================= */

#include "font.h"

/* Sadece ASCII yazdırılabilir karakterlerin temel bir bölümü (0-127) */
static const uint8_t font_8x8[128][8] = {
    [0 ... 31] = {0}, /* Control characters (boş) */
    ...
    ['t'] = {0x10, 0x3C, 0x10, 0x10, 0x10, 0x10, 0x0C, 0x00},
    ...
    ['~'] = {0x3A, 0x6E, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00},
    [127] = {0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00}
};

void font_draw_char(uint32_t x, uint32_t y, char c, uint32_t fg_color, uint32_t bg_color)
{
    if ((uint8_t)c > 127) c = '?'; /* Sadece ASCII destekleniyor */

    for (int row = 0; row < 8; row++) {
        uint8_t row_data = font_8x8[(uint8_t)c][row];
        for (int col = 0; col < 8; col++) {
            if ((row_data >> (7 - col)) & 1) {
                fb_putpixel(x + col, y + row, fg_color);
            } else if (bg_color != 0x00000000) { /* Saydam değilse arkaplanı boya */
...
void font_draw_string(uint32_t x, uint32_t y, const char *str, uint32_t fg_color, uint32_t bg_color)
{
    ...
    while (*str) {
        if (*str == '\n') {
            cursor_x = x;
            cursor_y += 10; /* 8 piksel font + 2 piksel satır arası */
        } else {
            font_draw_char(cursor_x, cursor_y, *str, fg_color, bg_color);
            cursor_x += 8; /* 8 piksel genişlik */
        }
        str++;
    }
}
